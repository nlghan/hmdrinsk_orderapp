...
const Login: React.FC<Props> = ({ navigation }) => {
  const { ensureActiveCart, checkGroupCart } = useCartStore();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.access_token) {
        ...
        // 🟡 Kiểm tra giỏ hàng nhóm đầu tiên
        const groupCartId = await checkGroupCart();
  
        // 🟢 Nếu không có giỏ nhóm, hoặc người dùng từ chối => fallback
        if (!groupCartId) {
      ...
      setLoading(false); // 🔁 Đặt ở cuối cùng để tránh bị lặp hoặc bỏ sót
    }
  };
