    ...
    useEffect(() => {
        ...
        const connectWebSocket = async () => {
            try {
                ...
                const ws = new WebSocket(`ws://192.168.1.13:1010/ws-raw?token=${encodedToken}&userId=${userId}`);
                ...
                ws.onmessage = (event) => {
                    try {
                        ...
                        if (
                            ['NEW_NOTIFICATION', 'NEW_GROUP_JOIN', 'MEMBER_LEFT_GROUP', 'MEMBER_KICKED', 'UPDATE_CART_LEADER'].includes(message.type)
                        ) {
                        ...
                        if (
                            ['NEW_GROUP_JOIN', 'MEMBER_LEFT_GROUP', 'MEMBER_KICKED', 'UPDATE_CART', 'CHECKOUT', 'UPDATE_CART_LEADER'].includes(message.type)
                        ) {
